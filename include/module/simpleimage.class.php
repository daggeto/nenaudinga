...
class SimpleImage {
   ...
   const LOGO_IMG_PATH = "img/logo_stamp.png";
   ...
   function insertLogo($image, $logo_src){
       $logo = $this->load($logo_src);

       imagecopy($image, $logo,self::OUTER_BORDER_WIDTH + self::INNER_BORDER_WIDTH , self::OUTER_BORDER_WIDTH + self::INNER_BORDER_WIDTH, 0, 0, imagesx($logo), imagesy($logo));
   }
   ...
   function createWithImage($img_src, $text){
        ...
        $this->image = $this->createBlankImage(self::WHOLE_IMAGE_WIDTH, $newHeight, 250, 242, 200);
        ...
        $this->insertCenteredText($this->image, $textParams, self::OUTER_BORDER_WIDTH, imagesy($upImage) + self::OUTER_BORDER_WIDTH * 2 + self::IMAGE_TEXT_SPAN + self::INNER_BORDER_WIDTH, $color, self::FONT_PATH);

        $this->insertLogo($this->image, self::LOGO_IMG_PATH);
